<template>
	<div v-for="(child, index) in childData" :key="index">
		<h1
			...
			class="mt-6 text-center text-3xl font-bold text-green-700 dark:text-green-500 md:text-3xl"
		>
			{{ child.name }}
		</h1>
		<div v-for="(division, dIndex) in child.divisions" :key="dIndex" class="-mt-2 flex flex-col p-8">
	...
	<div v-if="divisionsData.length === 0">
		...
		<h1 class="mt-6 text-center text-xl font-semibold text-main-fail-550 dark:text-main-fail-300">
